function [model, y_predict, cnt]= ryan5_run_inc_learning_with_legacy(model,n,updatePolicy,xnorm,ynorm,ynorm_reference,legacy_mdl)


   ynorm_legacy_larning_predict = predict(legacy_mdl,xnorm);
    ...
    for j = 1:nchunk
        ...
        elseif strcmp(updatePolicy,'use_yref_and_legacy_learning')
            model = updateMetricsAndFit(...
                model,...
                xnorm(idx,:),...
                (ynorm_reference(idx) + ynorm_legacy_larning_predict(idx))/2);
