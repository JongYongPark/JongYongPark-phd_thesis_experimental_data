function [idxSel,matchedRUL,idxCrossSel,idxSel_2nd,matchedRUL_2nd,idxCrossSel_2nd] = ...
    ryan5_util_find_similar_bat_with_rul_after_change( rul_data,idx_after_change,cap_after_change ) 
...
diff_lower = -1000;
for i=1:count 
    [val,idxCross] = min(abs(ryan5_battery_dataset(idxTrainValid(i)).QDischarge -  cap_after_change ));
...
matchedRUL = ryan5_battery_dataset(idxSel).cycle_life - idxCrossSel;
inTrain = any(idxTrain==idxSel);
inValid = any(idxValid==idxSel);
matchedRUL_2nd = ryan5_battery_dataset(idxSel_2nd).cycle_life - idxCrossSel_2nd;
