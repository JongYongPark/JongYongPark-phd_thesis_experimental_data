...
%test_semilogy();
%vlc_test_led_power() % LED 파워�? 대한 테스트 
...
%     end
    ...
    % 가장 마지막 채�?�?� parity channel으로 사용시
    fixed_parity_chan = true;
    % data channel �? �?러가 발�?할 경우만 수정
    % 아니면 어떤 채�?�?서 �?러가 발�?하�?��?��?� 수정
    fix_only_data_chan = false;
    
    if with_header
        % 실제 메세지�? 대한 것만 crc 패킷�? 넣는다.
        ...
        parity_sum = sum(rx_bin_mimo_fix_msg,1);    % 행(row) 열(column) �?서 1로 하면 �? 열�? 대해서 한다.
    else
        if fixed_parity_chan
            if fix_only_data_chan
                % rx_bin_mimo_fix_msg = rx_bin_mimo_fix_data ;
                parity_vec = rx_bin_mimo_fix_parity;
                parity_sum = sum(rx_bin_mimo_fix_data,1);    % 행(row) 열(column) �?서 1로 하면 �? 열�? 대해서 한다. 
                ...
                parity_sum = sum(correct_vec,1);    % 행(row) 열(column) �?서 1로 하면 �? 열�? 대해서 한다. 
                parity_vec_fixed = rem(parity_sum , 2 );
...
%     %snr=0; % some errors
    ...
    LED_loc_offset = 0.025; % defualt 5cm. �?�?�?서 2.5cm
...
%%
function vlc_test_packet_short(Fs)
...
%     use_h_function = false;
    ...
    %% Fs = 10000 - Impulse Response 가 �?�것�?� 기반으로 함.
    
    % 스위칭 딜레�?�가 거�?� 없다. 수신쪽 4.6 Volts ( Rz - SISO )
   Rb=100; 
   Fs=Rb*100;
    
    %% 여기서 부터 ISI 가 발�?한다.
    % 스위칭 딜레�?�거 커서 신호�? �?향�?� 미친다.수신쪽�?� �?�터심볼간�?� �?향�?� 미�?서 3.6~3.7 Volts 까지 나온다. ( Rz - SISO )
%      Rb=1000; 
%      Fs=Rb*10;
    
    % 스위칭 딜레�?�거 커서 신호�? �?향�?� 미친다.수신쪽�?� �?�터심볼간�?� �?향�?� 미�?서 3.2~3.5 Volts 까지 나온다. ( Rz - SISO )
%     Rb=1250; 
%     Fs=Rb*8;   

    %% 스위칭 딜레�?�거 커서 신호�? �?향�?� 미친다.수신쪽�?� �?�터심볼간�?� �?향�?� 미�?서 2.0~2.9 Volts 까지 나온다. ( Rz - SISO )
%     Rb=2500; 
%     Fs=Rb*4;   
    
    % �?�작안함 .디�?터가 최소 2 샘플�?�므로 �?�것�?� 안�?�다. ( Rz - SISO )
 	%Rb=5000; 
    %Fs=Rb*2;   
    
    %% LED 설정
	%LED_theta = 15; % 서로 �?향�?� 준다. 전달함수가 �?�미가 있다.
    LED_theta = 7; % 서로 �?향�?� 준지 않는다. 전달함수가 �?�미가 없다.
...
function [P_rec_panel,H] = vlc_rx_power_pola_panel_test(N_led,led_loc_offset,h,led_power,theta,show_figure)
    ...
    %% x,y = 10cm , �?�?으로 부터 5cm�? 중심 위치
    %% LED간 간격 = 5cm. �?�?으로 부터 25cm
    %% pd 가로 세로 0.5cm , pd_block 0.5cmx10개 = 5cm 
    H = zeros(2,2,4);
    %%%%%%% 실험 공간�?� 거리 설정 
    % 솔�?�패�?�?� 1x1 �?��?�고 하면 16 개�?� 패�?�?� 사용�?�다.
    lx=0.1;  % 10cm
    ly=0.1;  % 10cm
    lz=0.2;  % 20cm
    % 수신단�?� 높�?�
    ...
    % PD 가로/세로 길�?�
    Adet_1d = 0.005; % 0.5cm 
    % 솔�?�패�? 하나는 가로/세로로 PD 20개로 묶�?� - 여기서는 1M가 �?�다.
    ...
    %% 실험 - �? LED�? 따른 Pd ( 40*40) �?서�?� Power를 구한다.
    ...
    %% 모든 LED 가 켜져 있�?�때 �? Pd ( 40*40) �?� max power를 구한다.
...
%     P_rec_panel = zeros(4,4);
    ...
    %% 전달함수는 �? 패�?기준으로 어디�? �?향�?� 받는지�?� 비율�?�다. ( 4*4 )
    for i=1:16
        % 전달함수는 �? 패�?기준으로 어디�? �?향�?� 받는지�?� 비율�?�다.
       H_xx = P_rec_panel(:,:,i)./P_rec_panel_sum;   
       H(:,:,i) = H_xx; 
    end

    %% �? panel �?� 전달함수는 모�? 1 �?� �?�어야 한다.
    H_sum = sum (H,3);
    H_total = sum(H,'all');
...
% 나중�? shade 를 입력받아 출력하는 것으로 수정해야 한다.
    index=1;
...
function NRZ_Encoder_test()
    ...
    % 그리기 좋게 
    %sample_b = 10 
    % RZ �?� 있으므로 최소 2는 �?�어야 한다. 화면�? 보여지는 그래프는 삼�?형으로 보�?�다.
    sample_b = 2
...
function [ encodedData ] = Bin2Rz(inputData)
    ...
    for row = 1: rows
        for i = 1:1:nBits % count from max. size downwards with steps of 2
                encodedData(row,i*2-1) = inputData(row,i); % 앞�?� 비트는 입력 신호 그대로 
