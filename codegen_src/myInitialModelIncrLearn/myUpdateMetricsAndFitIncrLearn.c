    ...
    const double Y[5])
{
  ...
  if (!isInitialized_myInitialModelIncrLearn) {
	if(DEBUG_LOW) printf("\n isInitialized_myInitialModelIncrLearn == false ");	  
  ...
  if (incrementalModel->IsWarm) {
	...
	//-----------
	//for (i = 0; i < 20; i++)  if(DEBUG_LOW) printf("\n Beta[%d] = %.10f", i, incrementalModel->Impl.Beta[i]);
	i = 0;  if(DEBUG_LOW) printf("\n Beta[%d] = %.10f", i, incrementalModel->Impl.Beta[i]);
