function [result] = extract_feature_all_v1(B,cycleCountToIgnoreFromFirst,refCap)
    ...
    for i = 1:max_cycle
        ...
        if isequal(bcycle(i).type, 'charge') && ( i <=  max_cycle - last_charge_reverse_index  ) ...
            ...
            for ii = 1:length(bcycle(i).data.Current_measured)
                if ( cCC_end_current_skip == false) && ( bcycle(i).data.Current_measured(ii) < cCC_end_current )
                    cCC_end_current_skip = true;
                    break % end for 
                end
            end
            
            if cCC_end_current_skip == true 
            ...
            for ii = 1:length(bcycle(discharge_index).data.Current_measured)
                if ( dCC_end_current_skip == false) && ( bcycle(discharge_index).data.Current_measured(ii) > dCC_end_current )
                    dCC_end_current_skip = true;
                    break % end for 
                end
            end
            
            if dCC_end_current_skip == true 
...
result = [cap',cycleTimeHour',restTimeHour',restCycle',cTempMax',ohm',re_c',re_d',cycleNoChargeRe',cycleNoDischargeRe',dTempMax',cCcEndTimeSec',dCcEndTimeSec' ];
